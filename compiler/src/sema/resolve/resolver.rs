    ...
    pub fn new(context: GlobalContext<'a>) -> Resolver<'a> {
        Resolver {
            ...
            builin_types_bindings: PrimaryType::ALL
                ...
                .map(|&ty| {
                    (
                        context.intern_symbol(ty.name_str()),
...
impl<'a> Resolver<'a> {
    ...
    pub fn resolve_in_scope(
        ...
        super::scope_lookup::resolve_in_scope(scope, *name, namespace)
    }
...
impl<'a> Resolver<'a> {
    ...
    fn resolve_std_prelude_scope(&self, span: Span) -> Option<Scope<'a>> {
        ...
        let scope = [
            vec![
                Identifier {
                    symbol: self.context.intern_symbol(STD_PREFIX),
                    span,
                },
                Identifier {
                    symbol: self.context.intern_symbol("prelude"),
                    span,
                },
            ],
            vec![Identifier {
