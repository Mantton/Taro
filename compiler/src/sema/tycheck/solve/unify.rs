...
mod tests {
    ...
    use crate::{
        ...
        sema::{
            ...
            tycheck::{
                infer::InferCtx,
                test_support::{make_script_config, temp_dir, write_file},
                utils::ParamEnv,
            },
        },
        span::{FileID, Span},
    ...
    fn projection_equality_uses_the_resolved_inference_arguments() {
        let root = temp_dir("projection-equality");
        ...
        let config = make_script_config(&context, source, "projection-equality");
        let mut compiler = Compiler::new(&context, config);
        assert!(compiler.analyze_for_diagnostics(false).is_ok());
        let gcx = compiler.context;
        let assoc_id = gcx.with_type_database(config.index, |db| {
            *db.def_to_iface_def
                .values()
                .next()
                .unwrap()
