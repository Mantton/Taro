...
impl Literal {
    pub fn from(literal: taroc_ast::Literal) -> Result<Literal, String> {
        match literal.kind {
            taroc_ast::LiteralKind::Bool(v) => Ok(Literal::Bool(v)),
            taroc_ast::LiteralKind::String => Ok(Literal::String(literal.content)),
            taroc_ast::LiteralKind::Nil => Ok(Literal::Nil),
            taroc_ast::LiteralKind::Void => Ok(Literal::Void),
            ...
            taroc_ast::LiteralKind::Float => {
                match literal.content.as_str().parse::<f64>() {
                    Ok(v) => return Ok(Literal::Float(v)),
